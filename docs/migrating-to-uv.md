...
The deprecated `environment-core.yml` and `dse-check-env` console script retain the conda baseline shipped in 0.15.1. From 0.15.2, that baseline no longer tracks the current Python dependency floors because conda-forge does not provide the same compatible package set as PyPI. The baseline test checks that every retained package still has a current Python requirement with an equal or higher minimum. Passing `dse-check-env` does not establish compatibility with 0.15.2. Migrate to uv before adopting 0.15.2 or later; the legacy checker can be removed in a separate cleanup.
