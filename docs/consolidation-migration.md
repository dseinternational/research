...
Version 0.14.0 collects the library side of the downstream consolidation work merged in [PR #101](https://github.com/dseinternational/research/pull/101). The [upgrade notes](migrating-to-0.14.md) describe installation and the release sequence. Each downstream project can update its dependency and lockfile once, then replace local implementations through small adapters in the same upgrade PR. The library changes do not migrate downstream production code or establish that saved fits are compatible with changed implementation identities.
